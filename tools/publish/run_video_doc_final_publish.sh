...
if [ "${1:-}" = "" ]; then
  echo "Usage: tools/publish/run_video_doc_final_publish.sh RUN_DIR [--profile PROFILE] [--jobs N] [--finalize-only] [--skip-images] [--pdf|--skip-pdf] [--skip-send] [--to WECHAT_ID] [--long-png]" >&2
  exit 2
fi
...
regenerate_docs() {
  ...
  "$PYTHON_BIN" "$ROOT_DIR/tools/ops/run_local_model_stage.py" \
    ...
    --config config \
    --profile "$PROFILE" \
    -- "$PYTHON_BIN" "$ROOT_DIR/tools/publish/generate_chapter_deep_report.py" "$RUN_DIR" \
      --profile "$PROFILE" \
      --deep-v2 \
      --no-final-synthesis \
...
"$PYTHON_BIN" "$ROOT_DIR/tools/publish/augment_video_docs_images.py" "$RUN_DIR" "${augment_args[@]}"
...
if [ "$SKIP_PDF" -eq 0 ]; then
  "$ROOT_DIR/tools/publish/export_video_docs.sh" "$RUN_DIR" "${export_args[@]}"
fi
verify_counts
write_summary
