...
if [ "${1:-}" = "" ]; then
  echo "Usage: tools/publish/export_video_docs.sh <operation-manual-run-dir> [export-dir] [--final-only] [--jobs N] [--long-png]" >&2
  exit 2
fi
...
PDF_SCRIPT="${PDF_SCRIPT:-$ROOT_DIR/tools/publish/md_to_mobile_pdf.py}"
LONG_PNG_SCRIPT="${LONG_PNG_SCRIPT:-$ROOT_DIR/tools/publish/pdf_to_long_png.py}"
PREPARE_SCRIPT="${PREPARE_SCRIPT:-$ROOT_DIR/tools/publish/prepare_video_doc_export.py}"
