...
start_service() {
  ...
  for ((index = 0; index < count; index++)); do
    local gpu="${ids[index]}"
    [[ "${gpu}" == "3" ]] && { echo "GPU 3 is reserved" >&2; exit 2; }
  ...
  PYTHONPATH="${ROOT_DIR}" FIRERED_ASR2_WORKER_PORTS="${port_list}" FIRERED_ASR2_PROXY_PORT="${PROXY_PORT}" \
    setsid "${ROOT_DIR}/.venv/bin/python" "${ROOT_DIR}/tools/firered_asr2_p40_proxy.py" \
