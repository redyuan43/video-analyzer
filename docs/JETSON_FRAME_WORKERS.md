...
addresses in the default path. Set `JETSON_AGX_LAN_HOST=agx-lan` only after
adding a stable LAN DNS/DHCP hostname for AGX. Use
...
- if `JETSON_AGX_LAN_HOST` is set, it must point to a stable reachable LAN DNS
  name such as `agx-lan`, not a transient DHCP address;
- `ffmpeg` present;
