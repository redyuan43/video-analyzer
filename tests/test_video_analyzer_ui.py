...
class VideoAnalyzerUITests(unittest.TestCase):
    ...
    def test_home_page_contains_unified_video_link_workspace(self):
        ...
        self.assertIn('id="expandBilibiliParts"', html)
        ...
        self.assertIn("ASR 分块策略", main_js)
        self.assertIn("FireRedVAD", main_js)
        self.assertIn("data-asr-chunk-field=\"segmentationMode\"", main_js)
        self.assertIn("data-asr-chunk-field=\"workerCount\"", main_js)
        ...
        self.assertIn("按当前方案排队重跑核心分析", main_js)
        self.assertIn("按当前方案排队继续生成", main_js)
        self.assertIn("/stages/${rerunStage}/rerun", main_js)
        self.assertIn("failure_disposition?.category === 'resume_required'", main_js)
        self.assertIn("currentJob?.next_stage", main_js)
        self.assertIn("refresh_runtime_profile: true", main_js)
        self.assertIn("enqueue: true", main_js)
        self.assertIn("expand_bilibili_parts: nodes.expandBilibiliParts.checked", main_js)
        self.assertIn("function syncBilibiliCollectionOption()", main_js)
        self.assertIn("pendingUrls.length === 1 && isBilibiliVideoUrl", main_js)
        self.assertIn("sourceMode === 'file' || !supported", main_js)
        self.assertNotIn("checked = Boolean(candidate)", main_js)
        self.assertIn("完成但有失败", main_js)
        self.assertIn("|| Boolean(job.collection)", main_js)
        self.assertIn("const collectionDeleteLocked = Boolean(", main_js)
        self.assertIn("job.operator_read_only || collectionDeleteLocked", main_js)
        ...
        self.assertIn("const PREVIEW_PAGE_SIZE = 12;", main_js)
        self.assertIn("if (structureKey === previewGridStructureKey)", main_js)
        self.assertIn("pageJobs.forEach(syncPreviewCardState)", main_js)
        self.assertIn("? [currentJob, ...listedJobs]", main_js)
        self.assertIn("if (currentView === 'preview') {\n        renderTaskPreviewGrid(latestJobs);", main_js)
        self.assertIn("previousView === 'preview' && currentJob", main_js)
        self.assertIn("currentView === 'preview' && (!selectedJobId || currentJob)", main_js)
        self.assertIn('id="previewPagination"', html)
        self.assertIn('id="previewPreviousPage"', html)
        self.assertIn('id="previewNextPage"', html)
    ...
    def test_static_ui_marks_running_and_pending_states_visually(self):
        ...
        self.assertIn("consoleFlowTimerPlaceholder", js)
        self.assertIn("updateConsoleFlowNodeTimers", js)
        self.assertIn("consoleFlowNodeTimingLabel", js)
        self.assertIn("queueElapsedSeconds", js)
        self.assertIn("stageQueueMarkup", js)
        self.assertIn("data-mermaid-node-id", js)
        self.assertIn("已运行", js)
        self.assertIn("已等待", js)
        self.assertIn("阶段耗时", js)
        self.assertIn("stop-action", js)
        self.assertIn("play-action", js)
        self.assertIn("'stop'", js)
        self.assertIn(".stage-duration-summary", css)
        self.assertIn(".stage-timing", css)
        self.assertIn(".stage-queue-timing", css)
